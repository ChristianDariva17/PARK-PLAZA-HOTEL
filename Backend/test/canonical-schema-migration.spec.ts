...
describe('canonical supplier schema migration', () => {
  it('keeps supplier DDL in the versioned migration history', () => {
    ...
    expect(migration).toContain('ADD COLUMN "rating" integer DEFAULT 5');
    expect(migration).toContain('ADD COLUMN "rating_notes" text');
    expect(migration).toContain('CREATE TABLE "purchase_orders"');
    ...
    expect(migration).toContain('idx_purchase_orders_property_isolation');
  });
