...
import { parseFolioChargeDto, parseFolioEntryId, parseFolioPage, parseFolioPaymentDto, parseFolioReversalDto } from './folio.dto.js';

@Controller('stays/:stayId/folio')
export class FolioController {
  constructor(private readonly folios: FolioService) {}
  @Get() @RequirePermissions('stays.read') get(@Param('stayId') stayId: string, @Query() query: unknown, @CurrentAccount() actor: AuthenticatedAccount) { return this.folios.get(actor.propertyId, parseStayId(stayId), parseFolioPage(query)); }
