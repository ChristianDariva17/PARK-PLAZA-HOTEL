...
const admittedCustomerContract = (route, endpoint, dto, idempotency = 'not-applicable', invalidates = [], session = 'customer', permission = 'customer-session') => Object.freeze({
  route,
  endpoint,
  dto,
  session,
...
const admittedCustomerContracts = Object.freeze({
  '/': admittedCustomerContract('/', 'derived:public-experience', 'PublicCustomerExperienceTransport', 'not-applicable', [], 'public', 'public-catalog'),
...
export function isCustomerContractAdmitted(route) {
  ...
  return Boolean(
    ...
    && ['customer', 'public'].includes(matrix.session) && matrix.permission && matrix.propertyScope === 'server-enforced'
