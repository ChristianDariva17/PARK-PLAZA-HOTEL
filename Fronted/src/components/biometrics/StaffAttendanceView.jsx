...
export function StaffAttendanceView({ notify }) {
  ...
  return <div className="view-container">
    <PageHeader metadata="Biometría ZK9500 preservada · vía manual adicional" title="Personal y asistencia" description="Directorio, turnos y asistencia manual auditada sin reemplazar ni debilitar la validación biométrica." action={<PermissionButton actionType="STAFF_CREATE" className="btn btn-primary" onClick={() => setEditor(null)}>Registrar personal</PermissionButton>} />
    ...
    <TabPanel active={tab === 'directory'} label="Directorio y biometría"><div className="staff-card-grid">{state.staff.map((person) => <article className="card staff-card" key={person.id}><div className="row-between"><div><span className="eyebrow">{person.id} · DNI {person.documentNumber}</span><h3>{person.name}</h3><p>{person.role} · {person.area}</p></div><StatusBadge>{person.status}</StatusBadge></div><DetailGrid compact items={[{ label: 'Turno', value: person.shift }, { label: 'Asistencia', value: person.attendance }, { label: 'Horas extra', value: `${person.overtimeHours} h` }, { label: 'Correo', value: person.email }]} />{person.status !== 'Archivado' && (can(PERMISSIONS.staffBiometric) || can(PERMISSIONS.staffAttendance)) ? <BiometricPanel subjectType="employee" subjectId={person.id} subjectName={person.name} reference={person.biometric?.templateReference} canEnroll={can(PERMISSIONS.staffBiometric)} canVerify={can(PERMISSIONS.staffBiometric) && can(PERMISSIONS.staffAttendance)} onEnrolled={(result) => execute({ type: 'BIOMETRIC_ENROLLED', subjectType: 'employee', subjectId: person.id, ...result })} onVerified={(result) => biometricResults.current.set(person.id, result)} onAttempt={(attempt) => { execute({ type: 'BIOMETRIC_ATTEMPT', subjectType: 'employee', subjectId: person.id, ...attempt }); const result = biometricResults.current.get(person.id); if (attempt.kind === 'verify' && attempt.matched && result?.matched) execute({ type: 'STAFF_ATTENDANCE_VERIFIED', staffId: person.id, matched: true, score: result.score, templateReference: result.templateReference, requestId: attempt.operationId }); biometricResults.current.delete(person.id); }} /> : null}<div className="inline-actions">{person.status !== 'Archivado' ? <><PermissionButton actionType="STAFF_UPDATE" className="btn btn-outline" onClick={() => setEditor(person)}>Editar</PermissionButton><PermissionButton actionType="STAFF_ATTENDANCE_MANUAL" className="btn btn-outline" onClick={() => setManualId(person.id)}>Asistencia manual</PermissionButton></> : null}<PermissionButton actionType={person.status === 'Archivado' ? 'STAFF_REACTIVATE' : 'STAFF_ARCHIVE'} className={person.status === 'Archivado' ? 'btn btn-primary' : 'btn btn-danger'} onClick={() => { setReason(''); setOperation({ kind: 'staff', record: person }); }}>{person.status === 'Archivado' ? 'Reactivar' : 'Archivar'}</PermissionButton></div></article>)}</div></TabPanel>
    <TabPanel active={tab === 'shifts'} label="Turnos"><SectionHeader title="Turnos programados" action={<PermissionButton actionType="SHIFT_CREATE" className="btn btn-primary" onClick={() => setShiftEditor(null)}>Crear turno</PermissionButton>} /><DataTable caption="Turnos con validación de solapamiento" columns={['Turno', 'Persona', 'Fecha', 'Horario', 'Estado', 'Acciones']}>{state.staffShifts.map((shift) => <tr key={shift.id}><td>{shift.id}</td><td>{state.staff.find((item) => item.id === shift.staffId)?.name}</td><td>{shift.date}</td><td>{shift.startTime}-{shift.endTime}</td><td><StatusBadge>{shift.status}</StatusBadge></td><td><div className="inline-actions">{shift.status !== 'Cancelado' ? <><PermissionButton actionType="SHIFT_UPDATE" className="btn btn-outline" onClick={() => setShiftEditor(shift)}>Editar</PermissionButton><PermissionButton actionType="SHIFT_CANCEL" className="btn btn-danger" onClick={() => { setReason(''); setOperation({ kind: 'shift', record: shift }); }}>Cancelar</PermissionButton></> : null}</div></td></tr>)}</DataTable></TabPanel>
    ...
    <Dialog open={editor !== undefined} onClose={() => setEditor(undefined)} title={editor ? `Editar ${editor.name}` : 'Registrar personal'}><PermissionGate actionType={editor ? 'STAFF_UPDATE' : 'STAFF_CREATE'}><StaffEditor person={editor || null} onClose={() => setEditor(undefined)} notify={notify} /></PermissionGate></Dialog>
    <Dialog open={shiftEditor !== undefined} onClose={() => setShiftEditor(undefined)} title={shiftEditor ? `Editar ${shiftEditor.id}` : 'Crear turno'}><PermissionGate actionType={shiftEditor ? 'SHIFT_UPDATE' : 'SHIFT_CREATE'}><ShiftEditor shift={shiftEditor || null} onClose={() => setShiftEditor(undefined)} notify={notify} /></PermissionGate></Dialog>
    <Dialog open={Boolean(manualPerson)} onClose={() => setManualId(null)} title={manualPerson ? `Asistencia manual de ${manualPerson.name}` : 'Asistencia manual'}><PermissionGate actionType="STAFF_ATTENDANCE_MANUAL">{manualPerson ? <ManualAttendance person={manualPerson} onClose={() => setManualId(null)} notify={notify} /> : null}</PermissionGate></Dialog>
    <Dialog open={Boolean(operation)} onClose={() => setOperation(null)} title={operation?.kind === 'shift' ? 'Cancelar turno' : operation?.record.status === 'Archivado' ? 'Reactivar personal' : 'Archivar personal'} description="El registro histórico no se elimina.">{operation ? <PermissionGate actionType={operation.kind === 'shift' ? 'SHIFT_CANCEL' : operation.record.status === 'Archivado' ? 'STAFF_REACTIVATE' : 'STAFF_ARCHIVE'}><div className="form-grid"><label className="span-2">Motivo<textarea required value={reason} onChange={(event) => setReason(event.target.value)} /></label><div className="form-actions span-2"><button className="btn btn-outline" onClick={() => setOperation(null)}>Volver</button><button className="btn btn-danger" disabled={!reason.trim()} onClick={applyOperation}>Confirmar</button></div></div></PermissionGate> : null}</Dialog>
  </div>;
}
